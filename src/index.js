...
function listarItensDisponiveis(dados, tipo) {
  ...
  disponiveis.forEach((item, i) => {
    let numero = i + 1;
    if (tipo === "Livro") {
      console.log(`${numero} - ${item.titulo} (Autor: ${item.autor})`);
    } else {
      console.log(`${numero} - ${item.titulo} (Diretor: ${item.diretor})`);
    }
  });
  console.log("0 - Cancelar");
  return disponiveis;
...
do {
  ...
  if (opcao === 1) {
    ...
    if (usuario.itens.length === 0) {
      console.log("O usuário não tem itens emprestados.");
      continue;
    }
